...
**Task**: Analyze the GO enrichment results for some of the single cell clusters you identified yesterday, which are included in the [functional_enrichment folder](https://github.com/dsobral/ADER/tree/master/material/functional_enrichment). Look at both the BP and the CC graph files for each cluster.
<br/>
...
<details><summary>Click here to see the answer</summary><p>
Cluster F = 1 (T Cell specific terms are a giveaway that these are mainly T cells)
Cluster Z = 5 ("phagocytosis" is a giveaway for macrophages -- this could also mean cluster 14, but interstitial macrophages are more involved in lung homeostasis)
Cluster X = 6 (This one is trickier as you have "positive regulation of T Cell activation" in the BP graph, but "B cell receptor complex" in the CC graph clearly identifies these as B cells)
Cluster Q = 7 (The abundance of terms related to collagen and cell adhesion is a dead giveaway for stromal cells)
Cluster E = 8 (This one is not trivial, but "positive regulation of adaptive immune response" and "response to external biotic stimulus" point towards dendritic cells)
Cluster B = 10 (Again, not trivial, but the abundance of "viral" BP terms suggests plasmacytoid dendritic cell, which are in majority in this cluster)
Cluster T = 13 (Here several "neutrophil" BP terms leave no room for doubt that this cluster has neutrophils)
Cluster N = 15 (The "natural killer cell" specific BP term clearly identifies these as NK cells)
</p></details>
<br/>
